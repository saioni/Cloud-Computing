...
def create_ride():
        if (request.method == 'POST'):
                ...
                if user_in_db.text:
                        ...
                        ride = requests.post('http://localhost:8000/api/v1/db/write',json=query)
...
def del_ride(ride_id):
        if(request.method=='DELETE'):
                ...
                query = {"op":4}
                del_user = requests.post('http://localhost:8000/api/v1/db/write',json={"query":query})
...
def join_ride(ride_id):
        if (request.method=='POST'):
                ...
                if not user_in_db and not rideid_in_db:
                        ...
                        join_ride = requests.post('http://localhost:8000/api/v1/db/write',json=query)
                        s={}
                        res=JoinRide.query.filter_by(username=username).first()
